mta_setPedControlState = setPedControlState
function setPedControlState(ped,key,state)
	mta_setPedControlState(ped,key,state)
end
...
addCommandHandler("scena",function()
	...
	bindKey("v","down",function()
		applyToAllPeds(function(pedT)
			triggerServerEvent("killerPed",root,pedT.ped)
		end,true)
	end)
	...
	bindKey("num_3","both",function(key,state)
		applyToAllPeds(function(pedT)
			if pedT.fireTimer then killTimer(fireTimer) end
...
function setPedLookAtLookTarget(pedTab)
	local pedT = pedTab
	--outputChatBox(tostring(Vector3(pedT.lookTarget[1], pedT.lookTarget[2], pedT.lookTarget[3])))
...
--------------------------Sync Peds
	...
	function syncPeds()
		...
		if #pedsData > 0 then triggerLatentServerEvent("pedsSync",2*1024*1024,root,pedsData) end
	end
	setTimer(syncPeds,60,0)
