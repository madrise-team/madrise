// 
// file: primitive3D_specularLight.fx
// version: v1.6
// author: Ren712
//
...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
    ...
    // fixed direction or camera to light source
    float3 lightDir = bFixedDirection == true ? normalize(sLightDir) : -vLight;
	
    // Blinn half angle	
    float3 h = normalize(normalize(sViewInverse[3].xyz - worldPos) - lightDir);
    float specLighting = pow(saturate(dot(h, worldNormal)), sSpecularPower);	

    // compute NdotL
    float NdotL = saturate(max(0.0f, specLighting));
...
// Techniques
//--------------------------------------------------------------------------------------
technique dxDrawPrimitive3DSpecularLight
{
  pass P0
  {
    ZEnable = false;
...
technique dxDrawPrimitive3DSpecularLight_fallback
{
  pass P0
  {
    ZEnable = true;
