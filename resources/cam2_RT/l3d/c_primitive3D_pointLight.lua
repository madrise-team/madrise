--  
-- file: c_primitive3D_pointLight.lua
-- version: v1.6
-- author: Ren712
--
-- include: fx/primitive3D_pointLight.fx, c_common.lua

local scx,scy = guiGetScreenSize ()

CPrmLightPoint = { }
CPrmLightPoint.__index = CPrmLightPoint
 
-- worldPosition Vector3() , attenuation Vector1(0-n), color Vector4(0-255,0-255,0-255,0-255) 
function CPrmLightPoint: create(pos, atten, col )
	local scX, scY = guiGetScreenSize()
	
	local cShader = {
		shader = DxShader( "fx/primitive3D_pointLight.fx" ),
...
function CPrmLightPoint: setTesselationByDistance()
...
function CPrmLightPoint: setObjectTesselation( tess )
	if self.shader then
		self.shapeTess = tess
	end
end

function CPrmLightPoint: setDistFade( distFade )
	if self.shader then
...
function CPrmLightPoint: setPosition( pos )
	if self.shader then
...
function CPrmLightPoint: setAttenuation( atten )
	if self.shader then
...
function CPrmLightPoint: setAttenuationPower( attenPow )
	if self.shader then
...
function CPrmLightPoint: setColor( col )
	if self.shader then
...
function CPrmLightPoint: getObjectToCameraAngle()
...
function CPrmLightPoint: getDistanceFromViewAngle( inAngle )
	if self.shader then
...
function CPrmLightPoint: destroy()
	if self.shader then
		self.shader:destroy()
		self.shader = nil
	end
