...
function onDataChange(theKey, oldValue, newValue)
    if (getElementType(source) == "player") and source == localPlayer and (theKey == "snegmod") then
...
--addCommandHandler("12371263781236788769123867912876938679128673123",showSnowToggleBind)
...
function drawSnow()	
if (getElementData (localPlayer, "hideTable") or {}).sneg then return true end
				...
				--	outputDebugString(string.format("Flake removed. %.1f %.1f %.1f",flake.x,flake.y,flake.z))
					...
					if ground[gx][gy] and (flake.z+lz) > ground[gx][gy] then
						...
						if draw_x and draw_y then
							dxDrawImageSection(draw_x, draw_y, flake.size, flake.size, flake.section[1], flake.section[2], 32, 32, "flakes/real_tile.png", flake.rot, 0, 0, tocolor(222,235,255,flake.alpha))
