--  
-- file: c_primitive3D_pointLightNoNRM.lua
-- version: v1.6
-- author: Ren712
--
-- include: fx/primitive3D_pointLightNoNRM.fx, c_common.lua

local scx,scy = guiGetScreenSize ()
   
CPrmLightPointNoNRM = { }
CPrmLightPointNoNRM.__index = CPrmLightPointNoNRM
 
-- worldPosition Vector3() , attenuation Vector1(0-n), color Vector4(0-255,0-255,0-255,0-255) 
function CPrmLightPointNoNRM: create(pos, atten, col )
	local scX, scY = guiGetScreenSize()
	
	local cShader = {
		shader = DxShader( "fx/primitive3D_pointLightNoNRM.fx" ),
...
function CPrmLightPointNoNRM: setTesselationByDistance()
...
function CPrmLightPointNoNRM: setTesselationSwitchDistance( tessDis )
	if self.shader then
...
function CPrmLightPointNoNRM: setObjectTesselation( tess )
	if self.shader then
		self.shapeTess = tess
		self:setTesselationByDistance()
	end
...
function CPrmLightPointNoNRM: setDistFade( distFade )
	if self.shader then
...
function CPrmLightPointNoNRM: setPosition( pos )
	if self.shader then
...
function CPrmLightPointNoNRM: setAttenuation( atten )
	if self.shader then
...
function CPrmLightPointNoNRM: setAttenuationPower( attenPow )
	if self.shader then
...
function CPrmLightPointNoNRM: setColor( col )
	if self.shader then
...
function CPrmLightPointNoNRM: getObjectToCameraAngle()
...
function CPrmLightPointNoNRM: getDistanceFromViewAngle( inAngle )
	if self.shader then
...
function CPrmLightPointNoNRM: draw()
	if self.shader then
...
function CPrmLightPointNoNRM: destroy()
	if self.shader then
		self.shader:destroy()
		self.shader = nil
	end
