--  
-- file: c_primitive3D_spotLight.lua
-- version: v1.6
-- author: Ren712
--
-- include: fx/primitive3D_spotLight.fx, c_common.lua

local scx,scy = guiGetScreenSize ()

CPrmLightSpot = { }
CPrmLightSpot.__index = CPrmLightSpot

-- worldPosition Vector3() , attenuation Vector1(0-n), color Vector4(0-255,0-255,0-255,0-255) 
function CPrmLightSpot: create(pos, atten, col )
	local scX, scY = guiGetScreenSize()
	
	local cShader = {
		shader = DxShader( "fx/primitive3D_spotLight.fx" ),
...
function CPrmLightSpot: setTesselationByDistance()
...
function CPrmLightSpot: setTesselationSwitchDistance( tessDis )
	if self.shader then
...
function CPrmLightSpot: setObjectTesselation( tess )
	if self.shader then
		self.shapeTess = tess
		self:setTesselationByDistance()
	end
...
function CPrmLightSpot: setTheta( thetaVal )
	if self.shader then
		self.theta = thetaVal
...
function CPrmLightSpot: setPhi( phiVal )
	if self.shader then
		self.phi = phiVal 
...
function CPrmLightSpot: setFalloff( falloff )
	if self.shader then
...
function CPrmLightSpot: setDirection( dir )
	if self.shader then
...
function CPrmLightSpot: setRotation( rotDeg )
	if self.shader then
...
function CPrmLightSpot: setDistFade( distFade )
	if self.shader then
...
function CPrmLightSpot: setPosition( pos )
	if self.shader then
...
function CPrmLightSpot: setAttenuation( atten )
	if self.shader then
...
function CPrmLightSpot: setAttenuationPower( attenPow )
	if self.shader then
...
function CPrmLightSpot: setColor( col )
	if self.shader then
...
function CPrmLightSpot: getObjectToCameraAngle()
...
function CPrmLightSpot: getDistanceFromViewAngle( inAngle )
	if self.shader then
...
function CPrmLightSpot: destroy()
	if self.shader then
		self.shader:destroy()
		self.shader = nil
	end
