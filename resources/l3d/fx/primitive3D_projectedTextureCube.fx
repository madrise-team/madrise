// 
// file: primitive3D_projectedTectureCube.fx
// version: v1.6
// author: Ren712
//
...
float3 sLightRotationSpeed = float3(0,0,0);
...
// Sampler 
//--------------------------------------------------------------------------------------
samplerCUBE SamplerCubeTexture = sampler_state
{
    Texture = (sCubeTexture);
    AddressU = Border;
    AddressV = Border;
...
PSInput VertexShaderFunctionNoDB(VSInput VS)
{
    ...
    PS.WorldPos.xyz += 1.5 * mul(normalize(sViewInverse[3].xyz - sLightPosition), (float3x3)gViewMainScene).xyz;
...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
    ...
    if ((sLightRotationSpeed.x != 0) || (sLightRotationSpeed.y != 0) || (sLightRotationSpeed.z != 0)) 
        lightRotation += float3(fmod(gTime * sLightRotationSpeed.x, 2 * PI), fmod(gTime * sLightRotationSpeed.y, 2 * PI), 
            fmod(gTime * sLightRotationSpeed.z, 2 * PI));

    // create rotation matrix (for the cube texture)	
    float3x3 sRotation = (float3x3)createWorldMatrix(sLightPosition, lightRotation);
	
    // recreate world normal from depth
    float3 viewNormal = GetNormalFromDepth(TexProj.xy, PS.UvToView);
    float3 worldNormal = mul(viewNormal, (float3x3)sViewInverse);
    ...
    float3 TexCoord = mul(sRotation, -vLight);
	
    // get cube texture
    float4 texel = texCUBE(SamplerCubeTexture, TexCoord.xzy);
    finalColor *= texel;
...
{	
    ...
    // apply diffuse color
    finalColor.a *= saturate(fAttenuation);
	
    // set diffuse color
    finalColor.a *= saturate(PS.DistFade);
	
    // compute the direction to the light
    float3 vLight = normalize(sLightPosition - PS.WorldPos);
...
// Techniques
//--------------------------------------------------------------------------------------
technique dxDrawPrimitive3DProjectedTextureCube
{
  pass P0
  {
    ZEnable = false;
...
technique dxDrawPrimitive3DProjectedTextureCube_fallback
{
  pass P0
  {
    ZEnable = true;
