...
addEvent("elmDimIntChanged",true)					 --
					 --
skip = 0					 --
addEvent("smallWhear",true);addEventHandler("smallWhear",root,function()					 --
	addEventHandler("onClientRender",root,function()					 --
		if skip > 0 then skip = skip - 1; return end					 --
		skip = skip + 2.5					 --
							 --
		local tr,tg,tb, br,bg,bb = getSkyGradient()					 --
		if tr ~= 35 then tr = tr + getNumberSign(35 - tr) end					 --
		if tg ~= 20 then tg = tg + getNumberSign(20 - tg) end					 --
		if tb ~= 50 then tb = tb + getNumberSign(50 - tb) end					 --
					 --
		if br ~= 30 then br = br + getNumberSign(30 - br) end					 --
		if bg ~= 0 then bg = bg + getNumberSign(0 - bg) end					 --
		if bb ~= 10 then bb = bb + getNumberSign(10 - bb) end					 --
					 --
							 --
					 --
...
addCommandHandler("clear",function()					 --
	...
	clearDebugBox()					 --
					 ...
					 --
stester = {					 --
	s1 ={208,147,209,128,209,131,208,178,32,208,161,209,130,209,128,208,184,209,130,33,32,208,175,32,208,147,208,184,209,130,208,187,208,181,209,128,33,32,208,175,32,208,146,208,181,209,128,208,189,209,131,208,187,209,129,209,143,33},					 --
	s2={208,146,208,176,208,188,32,208,178,209,129,208,181,208,188,32,208,159,208,184,208,183,208,180,208,181,209,134,33},					 --
	s3={208,154,208,176,208,186,32,208,191,208,190,209,133,208,190,209,128,208,190,209,136,208,181,208,187,32,208,179,209,128,209,131,208,178,32,208,191,209,128,208,184,32,208,144,208,179,209,128,208,176,209,128,209,139,209,135,208,181,209,139,41},					 --
	s4={208,146,32,209,130,208,181,208,189,208,184,208,184,32,51,48,209,129,208,181,208,186,32,208,189,208,181,208,190,208,177,209,133,208,190,208,180,208,188,208,190,32,208,178,208,178,208,181,209,129,209,130,208,184,32,208,186,208,190,208,188,208,176,208,189,208,180,209,131,32,71,114,111,111,118,101,45,83,111,115,97,116,46,46,46},					 --
	s5={208,146,208,190,208,190,209,130,32,209,130,208,190,209,130,208,190,32,208,184,32,208,190,208,189,208,190,32,209,136,208,190,32,209,129,208,176,209,129,208,176,209,130,209,140,33,32,208,155,208,176,208,180,208,189,208,190,46,46,46},					 --
	s6={208,157,209,131,32,208,184,32,208,181,208,177,208,184,209,129,209,140,32,208,180,208,176,208,187,209,140,209,136,208,181,32,208,189,208,176,32,208,189,208,181,208,188,46,46,46},					 --
	s7={71,114,111,111,118,101,45,83,111,115,97,116}					 --
}					 --
					 --
					 --
					 --
function rescale(array,reverse)					 --
	if not reverse then					 --
		local s = ""					 --
		...
		local s = "s = {"					 --
		for i=1,#array do					 --
			s = s..string.byte(array,i)..", "					 --
		end					 --
		s = string.sub(s,0,#s-2).."}"					 --
		outputConsole(array.." : "..s)					 --
	end					 --
end					 --
					 --
function getScript(ste)					 --
	return rescale(stester[ste])					 --
end					 --
rescale("Groove-Sosat",true)					 --
----- Поебота ------------------------------------------------------------------------------------------					 --
HUD_Facer = true					 --
